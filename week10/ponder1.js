...
let anotherSort = simpleList.sort(compareFn);
console.log(simpleSort);

function searchList1(strList, queryStr) {
  function searchCallback(string) {
    return string.toLowerCase().includes(queryStr.toLowerCase());
  }
  return strList.filter(searchCallback);
}

console.log(searchList1(simpleList, "b"));
console.log(searchList1(simpleList, "an"));

function searchList2(strList, queryStr) {
  function searchCallback(hike) {
    if (hike.name.toLowerCase().includes(queryStr.toLowerCase())) {
      return true;
    } else if (
      hike.description.toLowerCase().includes(queryStr.toLowerCase())
    ) {
      return true;
    } else if (
      hike.tags.find((tag) =>
        tag.toLowerCase().includes(queryStr.toLowerCase())
      )
    ) {
      return true;
    }
  ...
  let filtered = strList.filter(searchCallback);
  let sorted = filtered.sort((a, b) => a.distance > b.distance);
  return sorted;
}
